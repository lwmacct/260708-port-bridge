...
Port Relay 由两个 companion extensions 组成，必须同时安装：
...
这里的 MCP server 运行在远程工作区侧，`127.0.0.1:9222` 会通过 Port Relay 转发到本机浏览器。
...
- Port Relay 会把本机端点能力暴露到远程环境。CDP 端口权限很高，建议优先绑定本机 `127.0.0.1`，远程侧也优先使用 Unix socket 或 `127.0.0.1`，不要绑定到 `0.0.0.0`。
