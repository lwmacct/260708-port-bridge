...
```text
Local Port Bridge: Show Local Status
```

如果 `localPortBridge.autoStart` 为 `true`，remote 扩展会在远程窗口启动后自动启动映射，并通知 local 扩展建立 control channel。
...
```text
port-bridge-local-0.4.260708.vsix
port-bridge-remote-0.4.260708.vsix
```
